...
class SpeakOptions(BaseModel):
	...
	text: str = Field(..., description='Text to generate audio from.')
	split_sentences: bool = Field(True, description='Whether to split sentences when text is too long.')
...
class SpeakResponse(BaseModel):
	...
	audio: str = Field(..., description='Audio data encoded in base64.')
	time: float = Field(..., description='Time taken to generate audio in seconds.')
...
class SpeakToFileResponse(BaseModel):
	...
	file_name: str = Field(..., description='File name of saved audio on server.')
	time: float = Field(..., description='Time taken to generate audio in seconds.')
